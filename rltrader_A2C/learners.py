...
class ReinforcementLearner:
    ...
    def run(
        ...
        for epoch in range(num_epoches):
            ...
            if (epoch == 999):
                self.save_models(epoch+1, self.stock_code)
            elif epoch == 1999:
                self.save_models(epoch+1, self.stock_code)
            elif epoch == 2999:
                self.save_models(epoch+1, self.stock_code)
    ...
    def save_models(self, num_epoch, stock_code):
        if self.value_network is not None and \
                self.value_network_path is not None:
            self.value_network.save_model(self.value_network_path + '_{}_{}.h5'.format(num_epoch, stock_code))
            print(self.value_network_path + '_{}_{}.h5'.format(num_epoch, stock_code))
        if self.policy_network is not None and \
                self.policy_network_path is not None:
            self.policy_network.save_model(self.policy_network_path + '_{}_{}.h5'.format(num_epoch, stock_code))
            print(self.policy_network_path + '_{}_{}.h5'.format(num_epoch, stock_code))
