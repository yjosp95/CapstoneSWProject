...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epoches', type=int, default=5000)
    ...
    parser.add_argument('--start_date', default='20160629')
    parser.add_argument('--end_date', default='20171228')
    ...
    for stock_code in args.stock_code:
        ...
        chart_data = pd.read_csv('data/v2/{}.csv'.format(stock_code))#, usecols=["date", "open", "high", "low", "close", "volume"])
        #chart_data.columns = map(str.lower, chart_data.columns) # to lower case

        # 기간 필터링
        #chart_data['date'] = chart_data['date'].str.replace('-', '')
        chart_data = chart_data[(chart_data['date'] >= 20160629) & (chart_data['date'] <= 20171231)]
        #chart_data = chart_data.dropna()

        # make training data set by using fs() in feature_select.py
        training_data = pd.read_csv('data/training/{}_{}.csv'.format(stock_code, 1))#, usecols=range(1,16))
        ...
        max_trading_unit = 3#max(int(1000000 / chart_data.iloc[-1]['close']), 1)
