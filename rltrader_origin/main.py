...
if __name__ == '__main__':
    ...
    for stock_code in args.stock_code:
        ...
        # chart_data = pd.read_csv('data/{}.csv'.format(stock_code), usecols=["Date", "Open", "High", "Low", "Close", "Volume"])
        # chart_data.columns = map(str.lower, chart_data.columns) # to lower case

        # # 기간 필터링
        # chart_data['date'] = chart_data['date'].str.replace('-', '')
        # chart_data = chart_data[(chart_data['date'] >= '20150501') & (chart_data['date'] <= '20171231')]
        # chart_data = chart_data.dropna()

        # # make training data set by using fs() in feature_select.py
        # training_data = pd.read_csv('pre_data/{}_{}.csv'.format(stock_code, 1), usecols=range(1,16))
