...
class Agent:
    ...
    def decide_action(self, policy_network, sample, epsilon):
        ...
        if np.random.rand() < epsilon:
            ...
            action = np.argmax(probs) # find index of max prob action => index
            confidence = probs[action] # confidence = prob of selected action =>  prob
        return action, confidence, exploration

    def validate_action(self, action):
        validity = True
        if action == Agent.ACTION_BUY:#buy
            # 적어도 1주를 살 수 있는지 확인
    ...
    def act(self, action, confidence):
        ...
        # 즉시 보상 판단 if profit, 1  , loss, -1
        self.immediate_reward = profitloss #1 if profitloss >= 0 else -1
        ...
        """if profitloss > self.delayed_reward_threshold:
            ...
            delayed_reward = 0"""
        return self.immediate_reward, 0
